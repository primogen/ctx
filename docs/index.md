...
Watches what you develop, walks a knowledge graph of **87,083 skill pages, 467 agents, 10,790 MCP servers, and 207 cataloged harnesses**, and recommends the
...
- A Karpathy 3-layer wiki at `~/.claude/skill-wiki/` is the single source
  of truth.
- **98,547 graph nodes** for the shipped skill/agent/MCP/harness
  inventory, including 87,083 skill pages
  and 207 harness pages under `entities/harnesses/`.
  Each page tracks tags, status, provenance, and usage where it applies.
- A **knowledge graph** (98,547 nodes, 2,632,883 edges) built from a
  13,191-node core plus 85,356 body-backed skill nodes.
  The graph has 52 Louvain communities and blends semantic cosine,
  tag overlap, and slug-token overlap; 85,356 skill bodies are
...
-   **Knowledge graph**
    ...
    98,547 shipped graph nodes: 13,191 curated skill/agent/MCP/harness nodes plus 85,356 body-backed skill nodes. The graph has
    2,632,883 weighted edges and 52 Louvain communities.
...
-   **Releases**
    ...
    **v1.0.14** — MIT, CI-matrixed (Ubuntu 3.12 plus Windows/macOS 3.11/3.12),
    ...
    and the full ~439 MiB wiki tarball with **98,547 nodes / 2,632,883 edges / 52 Louvain communities**.
