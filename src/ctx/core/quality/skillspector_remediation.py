...
def decide_record(record: SkillSpectorAuditRecord) -> RemediationDecision:
    ...
    if record.status in REMOVE_STATUSES:
        ...
        reason = "SkillSpector found issues; inspect body and remediate before keeping"
...
def render_markdown_plan(plan: dict[str, Any]) -> str:
    ...
    lines.append(
        "Remove actions are limited to records SkillSpector blocked or could not scan "
        "because no converted `SKILL.md` body exists. Finding records are review/remediate "
        "candidates and are not auto-deleted by this planner.",
    )
    return "\n".join(lines) + "\n"
